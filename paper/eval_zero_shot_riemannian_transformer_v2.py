"""Stage 2 (v2): zero-shot LOSO eval of the frozen trial-attention Riemannian Transformer.

TangentSpace is fit on the pooled SOURCE subjects only (never touches the held-out subject's
data). Trials are processed in fixed-size sets of N=32 through the frozen encoder (padding the
last, incomplete set by cycling trials from the same pool so every trial still gets an embedding
from a full-size N=32 forward pass, matching what the encoder was trained on).

    python -m paper.eval_zero_shot_riemannian_transformer_v2
"""
import csv
import os

import numpy as np
import torch
import torch.nn.functional as F
from pyriemann.estimation import Covariances
from pyriemann.tangentspace import TangentSpace
from torch.utils.data import DataLoader, TensorDataset

from paper import config
from paper.data import load_bnci_all
from paper.models.heads import MLPProbe
from paper.models.riemannian_transformer import TrialAttentionTransformer
from paper.utils import classification_metrics


def load_frozen_encoder(device):
    ckpt_path = os.path.join(config.CHECKPOINT_DIR, "pretrain_riemannian_transformer_v2_best.pt")
    ckpt = torch.load(ckpt_path, map_location=device)
    encoder = TrialAttentionTransformer(
        tangent_dim=config.RT_TANGENT_DIM, embed_dim=config.RT_EMBED_DIM,
        depth=config.RTV2_DEPTH, heads=config.RT_HEADS,
    ).to(device)
    encoder_state = {k[len("encoder."):]: v for k, v in ckpt["model"].items() if k.startswith("encoder.")}
    encoder.load_state_dict(encoder_state)
    encoder.eval()
    for p in encoder.parameters():
        p.requires_grad_(False)
    return encoder


def embed_all_trials(encoder, vecs, n_trials, device):
    """Returns (M, embed_dim) trial embeddings for all M trials in vecs, processed in full-size
    N=n_trials sets (padding the last set by cycling so every real trial gets embedded)."""
    m = len(vecs)
    n_chunks = int(np.ceil(m / n_trials))
    padded_len = n_chunks * n_trials
    idx = np.arange(padded_len) % m  # cycle to pad
    all_embeds = np.empty((padded_len, encoder.embed_dim), dtype=np.float32)

    with torch.no_grad():
        for c in range(n_chunks):
            sel = idx[c * n_trials:(c + 1) * n_trials]
            x = torch.from_numpy(vecs[sel]).unsqueeze(0).to(device)
            _, trial_out = encoder(x)
            all_embeds[c * n_trials:(c + 1) * n_trials] = trial_out.squeeze(0).cpu().numpy()

    # First occurrence of each original trial index gives its embedding.
    result = np.empty((m, encoder.embed_dim), dtype=np.float32)
    seen = np.zeros(m, dtype=bool)
    for pos, orig in enumerate(idx):
        if not seen[orig]:
            result[orig] = all_embeds[pos]
            seen[orig] = True
    return result


def train_mlp_head(feats, labels, n_classes, device):
    head = MLPProbe(config.RT_EMBED_DIM, n_classes, config.RT_HEAD_HIDDEN, config.RT_HEAD_DROPOUT).to(device)
    optimizer = torch.optim.Adam(head.parameters(), lr=config.PROBE_LR)
    loader = DataLoader(TensorDataset(feats, labels), batch_size=64, shuffle=True)
    head.train()
    for _ in range(config.RT_HEAD_EPOCHS):
        for feat, label in loader:
            feat, label = feat.to(device), label.to(device)
            optimizer.zero_grad()
            loss = F.cross_entropy(head(feat), label)
            loss.backward()
            optimizer.step()
    return head


def run(subjects=None, device: str = None):
    config.set_seed(config.SEED)
    device = device or ("cuda" if torch.cuda.is_available() else "cpu")
    subjects = subjects or config.EVAL_SUBJECTS
    n_trials = config.RTV2_N_TRIALS

    per_subject, label_encoder = load_bnci_all(subjects, align="ea")
    n_classes = len(label_encoder.classes_)
    encoder = load_frozen_encoder(device)

    fold_results = []
    for held_out in subjects:
        source_subjects = [s for s in subjects if s != held_out]
        source_X = np.concatenate([per_subject[s][0] for s in source_subjects], axis=0).astype(np.float64)
        source_y = np.concatenate([per_subject[s][1] for s in source_subjects], axis=0)
        target_X, target_y = per_subject[held_out]
        target_X = target_X.astype(np.float64)

        cov = Covariances(estimator="lwf")
        source_covs = cov.fit_transform(source_X)
        target_covs = cov.transform(target_X)

        ts = TangentSpace(metric="riemann")
        source_vecs = ts.fit_transform(source_covs).astype(np.float32)
        target_vecs = ts.transform(target_covs).astype(np.float32)

        source_feats = torch.from_numpy(embed_all_trials(encoder, source_vecs, n_trials, device))
        target_feats = torch.from_numpy(embed_all_trials(encoder, target_vecs, n_trials, device))

        head = train_mlp_head(source_feats, torch.from_numpy(source_y).long(), n_classes, device)
        head.eval()
        with torch.no_grad():
            pred = head(target_feats.to(device)).argmax(-1).cpu().numpy()

        metrics = classification_metrics(target_y, pred)
        metrics["held_out_subject"] = held_out
        fold_results.append(metrics)
        print(f"[riemannian_transformer_v2] held-out subject {held_out}: "
              f"acc={metrics['accuracy']:.4f} kappa={metrics['kappa']:.4f}")

    accs = np.array([r["accuracy"] for r in fold_results])
    kappas = np.array([r["kappa"] for r in fold_results])
    summary = {"variant": "riemannian_transformer_v2", "accuracy_mean": accs.mean(), "accuracy_std": accs.std(),
               "kappa_mean": kappas.mean(), "kappa_std": kappas.std()}
    print(f"[riemannian_transformer_v2] LOSO accuracy: {summary['accuracy_mean']:.4f} +/- "
          f"{summary['accuracy_std']:.4f}  kappa: {summary['kappa_mean']:.4f} +/- {summary['kappa_std']:.4f}")

    os.makedirs(config.RESULTS_DIR, exist_ok=True)
    with open(os.path.join(config.RESULTS_DIR, "zero_shot_riemannian_transformer_v2.csv"), "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["held_out_subject", "accuracy", "kappa"])
        writer.writeheader()
        for r in fold_results:
            writer.writerow(r)

    return summary, fold_results


if __name__ == "__main__":
    run()
